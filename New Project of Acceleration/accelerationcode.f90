...
DOUBLE PRECISION,ALLOCATABLE,DIMENSION(:)     :: NS_X,NS_Y,TS_X,TS_Y,TA_X,TA_Y,NA_X,NA_Y,A,TV,p_mean,x_up,cp_up,x_lo,cp_lo
...
DOUBLE PRECISION    :: R_MAX,R_LOCAL,M_LOCAL,MN,MAXM,MINM,RPLUSU,RMINUSU,MACH_FSTREAM,RE_FSTREAM
DOUBLE PRECISION    :: XG,YG,X_ZIG,X_ETAG,Y_ZIG,Y_ETAG,GRADZI_T,CORR_ACCN
DOUBLE PRECISION    :: GRAD_ETA,COEFF_ZI,COEFF_ETA,COEFF_ZI_ETA,DF_ZI,DF_ETA,DG_ZI,DG_ETA
DOUBLE PRECISION    :: DU_DX,DU_DY,DV_DX,DV_DY,DT_DX,DT_DY,DF_DX,DG_DY,S_X,S_Y,del_S_X,del_S_Y
DOUBLE PRECISION    :: DU_DZI,DV_DZI,DVN_DZI,DU_DETA,DV_DETA,DT_DZI,DT_DETA,T_START,T_END,x_le,y_le,DIST
...
DOUBLE PRECISION    :: CX,CY,CXP,CYP,CXV,CYV,CXD,CYD,CL,CD,CDP,CDV,CDD,ANUSS,FUN1,FUN2,V_zi,V_eta,DIS_ZI1,DIS_ZI2
DOUBLE PRECISION    :: QFLUX,QFLUXP,JACOB,JACOBP,DILAT,DILATP,GRAETA,GRAETAP,VORT,VORTP,dev,dev_max
DOUBLE PRECISION    :: PX,PY,DILATX,DILATY,VORTX,VORTY,HEATF,SHEAR,Nu_local,ML,p_min,p_max,cfl_eta_max
DOUBLE PRECISION    :: PXY,VORTXY,DILATXY,CXYP,CXYV,CXYD,CMP,CMV,CMD,CM,XX_MAX,UTP,UTPP,VAR1,VAR2,VAR3
DOUBLE PRECISION    :: wave_speed_zi1,wave_speed_zi2,wave_speed_eta1,wave_speed_eta2,wzi_max,weta_max,cfl_zi,cfl_eta,cfl_zi_max
...
INTEGER             :: I,J,L,W_L,IP,IPP,IM,IOS,LOOP_TRIGGER,SNAP_COUNTER,LOOP_INCR,N_SNAP,LOOP_START,LOOP_END,il
INTEGER             :: nstep,maxstep,st,np(2),N_INS,IS,KK,T_LIMIT
INTEGER             :: IC1,IC2,IC3,IC4,ITE,ITU1,ITU2,ITU3,ITU4,ITL1,ITL2,ITL3,ITL4
INTEGER             :: file_flag=0,file_write=0
...
!CHARACTER(LEN=100) :: OUTPUT
 ...
 CHARACTER(LEN=20)  :: CAMERA="ON"
 CHARACTER(LEN=20)  :: MACH_CAM="ON"
 CHARACTER(LEN=20)  :: CLUSTER="no"
 CHARACTER(LEN=10)  :: OUTFILE
 character (len=5)  :: xfile
 character (len=25) :: filename
! character (len=5)  :: xfile1
! character (len=25) :: filename1
 LOGICAL           ::  LOGIC1,LOGIC2
 LOGICAL           ::  INITIAL = .FALSE. 

!================================PARAMETER=============================================
DOUBLE PRECISION,PARAMETER :: THETA=0.0d0,PHI_R=-8.0d0,S_=110.0d0,GAMA=1.40d0,R=287.0d0,U_inf=1.0d0
DOUBLE PRECISION,PARAMETER :: DT=0.00001d0,gw=0.0d0,EPS1=0.0d0,TS=1.0d0+EPS1,T0=273.0d0,T_FS=300.0d0,ac=1.0d0	
DOUBLE PRECISION,PARAMETER :: A1=0.028109,A2=1.0739,A3=-0.099183,C1=0.012012,Cv=R/(GAMA-1)
DOUBLE PRECISION,PARAMETER :: C2=0.065283,C3=-0.015755,MACH=0.5d0,Cp=GAMA*Cv,Tr=T_FS/T0 
...
DOUBLE PRECISION,PARAMETER :: acx=-1.0d0, acy=0.0d0, FR_AX=5.0d0, FR_AY=1.0d0, S_XTH=2.0d0,S_YTH=0.0d0,MACH_TH=0.9d0
!---------------------------------------------------------------------------------------   
! acx,acy are the acceleration component directions of the airfoil (-1,0,+1)
! This also acts as a switch to activate the acceleration
!---------------------------------------------------------------------------------------    
    maxstep=50000200  ! MAXIMUM TIME LOOPS
...
!----------------ALLOCATION-------------------------
 ALLOCATE(X(np(1)+100,np(2)),Y(np(1)+100,np(2)),ZI_X(np(1)+100,np(2)),ZI_Y(np(1)+100,np(2)))
 ALLOCATE(ETA_X(np(1)+100,np(2)),ETA_Y(np(1)+100,np(2)),x_up(np(1)+100),x_lo(np(1)+100))
!-----------------------------------------
...
!========================================================================== 
! =========================================================================
!	Inserting some points around the trailing edge
! =========================================================================
   IF(CLUSTER=='yes')THEN
   
        N_INS=(ITL4+12)-(ITU4-6)
        ITE=ITE-(ITU4-6)+ITE
        IS=ITU4-8
        WRITE(*,*)N_INS
        DO KK=1,N_INS
         ...
         DO J=1,np(2) 
!============================================================================
!   PUSHING ARRAY ELEMENTS BY 1 INDEX ALONG ZI FOR ONE INSERTION FOR GIVEN ETA
!============================================================================
         DO L=np(1),IS+1,-1         
          ...
          ZI_X(L+1,J)=ZI_X(L,J)
          ZI_Y(L+1,J)=ZI_Y(L,J)
          ETA_X(L+1,J)=ETA_X(L,J)
          ETA_Y(L+1,J)=ETA_Y(L,J)
         END DO
 
!============================================================================
!     INSERTING THE GRID DATA FOR GIVEN ETA
!============================================================================				
          X(IS+1,J)=0.5*(X(IS,J)+X(IS+2,J))
          Y(IS+1,J)=0.5*(Y(IS,J)+Y(IS+2,J))
          ZI_X(IS+1,J)=0.5*(ZI_X(IS,J)+ZI_X(IS+2,J))
          ZI_Y(IS+1,J)=0.5*(ZI_Y(IS,J)+ZI_Y(IS+2,J))
          ETA_X(IS+1,J)=0.5*(ETA_X(IS,J)+ETA_X(IS+2,J))
          ETA_Y(IS+1,J)=0.5*(ETA_Y(IS,J)+ETA_Y(IS+2,J))
...
!     PAUSE
! artificial boundary cut at 60 non-dimensional distance
      ...
      y_le=y(i,j) 
      do j=2,np(2) 
        DIST=DSQRT((X(I,J)-X_LE)**2+(Y(I,J)-Y_LE)**2)
        IF(DIST.GE.R_MAX)GO TO 999
      end do
999   np(2)=J-1
      ETA=1.0/(np(2)-1)
...
OPEN(2,FILE="GRID_OUTPUT",STATUS='unknown')
...
!==================ALLOCATION==============================================
 ...
 ALLOCATE(SP(np(1),2))
 ALLOCATE(p_mean(np(1)),TV(np(1)),cp_up(np(1)),cp_lo(np(1)))
...
!======================Read OUTPUT File=====================
 ...
 READ(7,100) Time,nstep,S_X,S_Y,uinf,vinf
 READ(7,*) 
 READ(7,*)
 ...
 CLOSE(7)
 100	format(8X,F20.10,I9,F9.4,F9.4,F8.4,F8.4,1X)
  
 IF(CAMERA.EQ.'ON')THEN
        
  OPEN(3,FILE="CAMERA.DAT",STATUS='UNKNOWN')
!  WRITE(*,*)'HELLO' 
  READ(3,*)SNAP_COUNTER,T_START,T_END,N_SNAP,LOOP_TRIGGER,LOOP_INCR
  CLOSE(3)
        
  IF(SNAP_COUNTER.EQ.0)THEN
  LOOP_START = nstep + (T_START-TIME)/DT
  LOOP_END = nstep +(T_END-TIME)/DT
  LOOP_INCR = (LOOP_END-LOOP_START)/(N_SNAP-1)
  LOOP_TRIGGER = LOOP_START
  END IF
  END IF
 
  IF(MACH_CAM.EQ.'ON')THEN 
  OPEN(3,FILE="OUTPUT_MACH.DAT",STATUS='UNKNOWN')
  READ(3,*)file_flag,file_write
  CLOSE(3)
  ENDIF      
!  WRITE(*,*)LOOP_START,LOOP_END,LOOP_INCR,LOOP_TRIGGER
!  PAUSE
 !maxstep=nstep+40000
 write(*,*)maxstep,nstep,S_X,S_Y
! S_X=0.0d0
! S_Y=0.0d0
  GO TO 101
!======================================================================= 
...
!    IF(I==1)IM=np(1)-1  
!   -------------------------------------------------------------------
!     calculating the CFL numbers using contravariant velocities
!   -------------------------------------------------------------------
!    V_zi=(VAR(I,J,1)*ZI_X(I,J)+VAR(I,J,2)*ZI_Y(I,J))/DSQRT(ZI_X(I,J)**2+ZI_Y(I,J)**2)
!    V_eta=(VAR(I,J,1)*ETA_X(I,J)+VAR(I,J,2)*ETA_Y(I,J))/DSQRT(ETA_X(I,J)**2+ETA_Y(I,J)**2)
!    wave_speed_zi1=DABS(V_zi+(1./MACH)*DSQRT(VAR(I,J,3)))
!    wave_speed_zi2=DABS(V_zi-(1./MACH)*DSQRT(VAR(I,J,3)))
!    wzi_max=MAX(wave_speed_zi1,wave_speed_zi2)  
!    wave_speed_eta1=DABS(V_eta+(1./MACH)*DSQRT(VAR(I,J,3)))
!    wave_speed_eta2=DABS(V_eta-(1./MACH)*DSQRT(VAR(I,J,3)))
!    weta_max=MAX(wave_speed_eta1,wave_speed_eta2)
!    DIS_ZI1=DSQRT((X(IP,J)-X(I,J))**2+(Y(IP,J)-Y(I,J))**2)
!    DIS_ZI2=DSQRT((X(I,J)-X(IM,J))**2+(Y(I,J)-Y(IM,J))**2)
!    cfl_zi=2*wzi_max*dt/(DIS_ZI1+DIS_ZI2)
!    cfl_eta=weta_max*dt/eta 
!    if(cfl_zi.gt.cfl_zi_max)cfl_zi_max=cfl_zi 
!    if(cfl_eta.gt.cfl_eta_max)cfl_eta_max=cfl_eta 

!    END DO
!    END DO
!    write(*,*)cfl_zi_max,cfl_eta_max
!    PAUSE 
...
!-----------------------------------------------------------------------   
    S(I,J,1)=0.d0
    S(I,J,2)=-acx*ROW(I,J)/(FR_AX*FR_AX)
    S(I,J,3)=-gw*(ROW(I,J)-1)/(FR*FR)-acy*ROW(I,J)/(FR_AY*FR_AY)
    S(I,J,4)=gw*(1-ROW(I,J))*VAR(I,J,2)*BK*RE/(FR*FR)+BK*RE*ROW(I,J)*(-acx*VAR(I,J,1)/(FR_AX*FR_AX)-acy*VAR(I,J,2)/(FR_AY*FR_AY))
!-----------------------------------------------------------------------    
    DO L=1,4
...
!-----------------------------------------------------------------------
    ...
    SP(I,1)=-acx/((1+EPS1)*(FR_AX*FR_AX))
    SP(I,2)=gw*(EPS1/(1+EPS1))/(FR*FR)-acy/((1+EPS1)*(FR_AY*FR_AY))
    IF(I.EQ.1) THEN
    SP(np(1),1)=SP(1,1)
    SP(np(1),2)=SP(1,2)
    ENDIF
    ...
    CORR=gw*DN*(1.d0/PK)*G_ETA(2)/((1+EPS1)*FR*FR)
    CORR_ACCN=DN*(1.d0/PK)/(1+EPS1)
    CORR_ACCN=CORR_ACCN*((acx*G_ETA(1))/(FR_AX**2)+(acy*G_ETA(2))/(FR_AY**2)) 

    P(I,J)=(P(I,J+1)-DN*SUM1)/(1-CORR-CORR_ACCN)
    ROW(I,J)=((P(I,J)/PK)+1)/VAR(I,J,3)
...
!=======================================================================
 ...
 uinf=uinf-(acx/(FR_AX**2))*DT
 vinf=vinf-(acy/(FR_AY**2))*DT

 MACH_FSTREAM=MACH*DSQRT(uinf**2+vinf**2)
 RE_FSTREAM=RE*DSQRT(uinf**2+vinf**2)
...
OPEN(2,FILE='COEFF_his.DAT',STATUS='UNKNOWN',ACCESS='append')
   WRITE(2,'(7(e13.6,2x))')Time,CD,CL,ANUSS,MACH_FSTREAM,RE_FSTREAM
 CLOSE(2)
...
!-----------------------------------------------------------------------
       OPEN(7,FILE="surf_press_upper",STATUS='UNKNOWN')
       OPEN(8,FILE="surf_press_lower",STATUS='UNKNOWN')
       DO i=1,ITE
        x_up(i)=x(i,1)
        cp_up(i)=2*P(i,1)
!        write(*,*)il,i,x(i,1)
...
!        pause
       ...
       DO I=1,ITE
        WRITE(7,31)X_UP(I),CP_UP(I)
       END DO
       DO I=1,IL
        WRITE(8,31)X_LO(I),CP_LO(I)
       END DO
30     FORMAT(1(F10.5,2X),5(E16.8,2X))
31     FORMAT(f10.5,2X,E16.8)
       CLOSE(7)
       CLOSE(8)
    ...
    END IF  
        ...
        if (CAMERA.EQ.'ON') THEN 
	      ...
	      if (nstep.EQ.LOOP_TRIGGER) then
!          write(*,*)'check'
	      SNAP_COUNTER=SNAP_COUNTER+1
        write(xfile,"(I5.5)")SNAP_COUNTER
        filename='OUTPUT'//trim(xfile)//'.DAT'
        
        if (SNAP_COUNTER.eq.(N_SNAP+1))THEN
        CAMERA="OFF"
        goto 102
        ENDIF

        OPEN (1,FILE=filename,status='unknown')
        WRITE(1,*)'VARIABLES=,"X","Y","ROW","P","T","U","V","VORT","ML"'
        WRITE(1,*)"ZONE"
        WRITE(1,*)"I=",np(1)
        WRITE(1,*)"J=",np(2)
        ...
        WRITE(1,40)X(I,J),Y(I,J),ROW(I,J),P(I,J),VAR(I,J,3),VAR(I,J,1),VAR(I,J,2),VORT,ML
...
40     FORMAT(2(F10.5,2X),7(E16.8,2X))        
        
       filename='SURFACE'//trim(xfile)//'.DAT'
...
!=======================================================================
       ...
       DT_DETA=(4*VAR(I,2,3)-3*VAR(I,1,3)-VAR(I,3,3))/(2*ZI)
       Nu_local=-(1/EPS1)*DT_DETA*SQRT(ETA_X(I,1)**2+ETA_Y(I,1)**2)
       
       IF(EPS1.eq.0) Nu_local=0.0
       
!=======================================================================
       
       WRITE(2,20)(i-1)*ZI,VORT,DILAT,SHEAR,Nu_local,P(I,1)
       END DO
       CLOSE(2)

       filename='VORT'//trim(xfile)//'.DAT'
       OPEN (2,FILE=filename,status='unknown')             
       WRITE(2,*)'ZONE'
...
!       write(*,*)LOOP_INCR,LOOP_TRIGGER
!       PAUSE
       LOOP_TRIGGER=LOOP_TRIGGER+LOOP_INCR
...
!      write(*,*)LOOP_INCR,LOOP_TRIGGER
!       PAUSE
       filename='surf_press_upper'//trim(xfile)//'.DAT'
       OPEN(7,FILE=filename,STATUS='UNKNOWN')
       filename='surf_press_lower'//trim(xfile)//'.DAT'
...
!31     FORMAT(f10.5,2X,E16.8)
       ...
       OPEN(3,FILE="CAMERA.DAT",STATUS='UNKNOWN')
       WRITE(3,*)SNAP_COUNTER,T_START,T_END,N_SNAP,LOOP_TRIGGER,LOOP_INCR
...
!       IF(Time>=T_LIMIT)THEN
!       write(xfile1,"(I3)")T_LIMIT
!        filename1='OUTPUTS'//trim(xfile1)//'.DAT' 
! OPEN(7,FILE=filename1,STATUS='unknown')
! WRITE(7,103) 'TITLE ="',Time,nstep,S_X,S_Y,uinf,vinf,'"'
...
!~    ML(2) =MACH*V_CONT(I,J,2)/SQRT(VAR(I,J,3))
 ...
 103	format(A,F20.10,I9,f9.4,f9.4,f8.4,f8.4,A)
...
!RE_FSTREAM=RE*DSQRT(uinf**2+vinf**2)	
IF(MACH_CAM=='ON')THEN
IF(MACH_FSTREAM>=0.6.and.file_flag==0)then
 file_flag=1 
 OUTFILE='OUTPUT_6'
 file_write=1 
endif
IF(MACH_FSTREAM>=0.7.and.file_flag==1)then
 file_flag=2 
 OUTFILE='OUTPUT_7'
 file_write=1
endif
IF(MACH_FSTREAM>=0.8.and.file_flag==2)then
 file_flag=3 
 OUTFILE='OUTPUT_8'
 file_write=1
endif
...
!~    ML(2) =MACH*V_CONT(I,J,2)/SQRT(VAR(I,J,3))
  ...
  OPEN(3,FILE="OUTPUT_MACH.DAT",STATUS='UNKNOWN')
  WRITE(3,*)file_flag,file_write
  CLOSE(3)  
 ENDIF  ! This is basically mach camera  section

IF(mod(nstep,100)==0) THEN
OPEN(8,FILE='DISP.DAT',STATUS='UNKNOWN',ACCESS='append')
WRITE(8,"(7(f10.4,2x))")time,uinf,vinf,MACH_FSTREAM,RE_FSTREAM,S_X,S_Y
 CLOSE(8)
 ENDIF

!MODIFYING FREE STREAM VELOCITIES FOR ACCELERATION TERMS
	
IF(MACH_FSTREAM>=MACH_TH)THEN
GOTO 110
ENDIF
